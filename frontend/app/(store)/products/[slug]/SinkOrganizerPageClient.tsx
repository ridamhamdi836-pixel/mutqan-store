...
type SinkOrganizerPageClientProps = {
...
const PAGE = SINK_ORGANIZER_PAGE;
const OFFER_HEADING_ID = "sink-offer-heading";

export function SinkOrganizerPageClient({
  product,
  embedMode = "store",
}: SinkOrganizerPageClientProps) {
  const isUpsellPreview = embedMode === "upsell-preview";
  const config = PRODUCTS_CONFIG["sink-organizer"];
    ...
    <div className="bg-brand-background">
      ...
      <section className="cv-section product-section-pad page-x bg-brand-surface">
        <div className="max-w-content mx-auto max-w-3xl space-y-3 md:space-y-4 text-center">
          <h2 className="text-xl sm:text-2xl md:text-3xl font-extrabold text-brand-espresso">
            {PAGE.pipeCompatibility.title}
          </h2>
          <p className="text-sm text-brand-muted">{PAGE.pipeCompatibility.copy}</p>
          <SinkOrganizerMedia
            src={config.lifestyleSectionImage}
            alt={config.lifestyleSectionImageAlt}
            aspect={config.lifestyleSectionAspect ?? "4/3"}
            placeholder={PAGE.pipeCompatibility.placeholder}
          />
        </div>
      </section>
