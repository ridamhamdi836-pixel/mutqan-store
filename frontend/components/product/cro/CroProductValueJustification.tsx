...
import type {
  CroProductPageConfig,
  ValueJustificationIcon,
} from "@/types/cro-product-page";
...
type ValueSection = CroProductPageConfig["valueJustification"];
type Card = ValueSection["cards"][number];
...
type CroProductValueJustificationProps = {
  section: ValueSection;
};

export function CroProductValueJustification({
  section,
}: CroProductValueJustificationProps) {
  const featured = section.cards.find((c) => c.featured);
  const rest = section.cards.filter((c) => !c.featured);
    ...
    <section className="cv-section page-x py-8 md:py-12 relative overflow-hidden">
      ...
      <div className="relative max-w-content mx-auto space-y-5 md:space-y-7">
        ...
        {featured ? (
          <ValueCard
            card={featured}
            index={section.cards.indexOf(featured)}
            featuredStatLabel={section.featuredStatLabel}
